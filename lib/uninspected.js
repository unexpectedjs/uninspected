...
uninspected.inspect = function (obj, options) {
    ...
    return unexpected.createOutput(uninspected.outputFormat).appendInspected(obj, depth + 1).toString();
};
...
['log', 'info', 'warn', 'error'].forEach(function (methodName) {
    uninspected[methodName] = function () { // ...
        var output = uninspected.createOutput(isChrome ? 'coloredConsole' : uninspected.outputFormat);
